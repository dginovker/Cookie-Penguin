...
func get_cooldown_for_player(player_id: int) -> float:
    var weapon: ItemInstance = ItemManager.get_item_at(ItemLocation.new(ItemLocation.Type.PLAYER_GEAR, player_id, 0))
    if weapon == null:
        return 0.8
    return get_item_cooldown(weapon)

func get_item_cooldown(weapon: ItemInstance) -> float:
...
func get_bullet_name_for_player(player_id: int) -> String:
    var weapon: ItemInstance = ItemManager.get_item_at(ItemLocation.new(ItemLocation.Type.PLAYER_GEAR, player_id, 0))
    if weapon == null:
        return "tier_0_bullet.png"

    return get_bullet_name_for_weapon(weapon)

func get_bullet_name_for_weapon(weapon: ItemInstance) -> String:
    if weapon.item_type == 'tier_0_sword':
