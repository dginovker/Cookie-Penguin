...
@export var origin := Vector2.ZERO         # world XZ for pixel (0,0)
...
func _img_for_idx(idx:int)->Image:
...
# Enum-friendly helpers (enum is an int; its value is the layer index)
func weight_at(xz:Vector2, terrain:int)->float:
    var idx := terrain
    var p := _px(xz)
    var col := _img_for_idx(idx).get_pixelv(p)
    return _ch(col, idx % 3)
