...
func start_server() -> void:
    is_client = false
    server_version_hash = GameVersion.HASH
    server_version_count = GameVersion.COUNT
    print("Server version: ", server_version_hash, " (commit #", server_version_count, ")")
...
func _process(_dt: float) -> void:
    if signal_mp:
        ...
        if is_client and signal_mp.get_connection_status() == MultiplayerPeer.CONNECTION_CONNECTED and !ws_hello_sent:
            signal_mp.put_packet(JSON.stringify({ "type":"hello", "client_version_hash":GameVersion.HASH, "client_version_count":GameVersion.COUNT }).to_utf8_buffer()); ws_hello_sent = true
            #print("Sent hello")
...
func _server_handle_signal(ws_id: int, msg: Dictionary) -> void:
    ...
    match String(msg.get("type", "")):
        "hello":
            ...
            PlayerManager.players[peer_id] = PlayerManager.PlayerState.new()
            PlayerManager.players[peer_id].client_version = msg.client_version_count
            PlayerManager.players[peer_id].client_version_hash = msg.client_version_hash
...
func _client_handle_signal(msg: Dictionary) -> void:
    ...
    match String(msg.get("type", "")):
        ...
        "offer":
            ...
            var version_hash: String = msg.get("version_hash", "unknown")
            var version_count: int = int(msg.get("version_count", 0))
            Yeet.set_server_version(version_hash, version_count)
